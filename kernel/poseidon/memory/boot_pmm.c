...
** period of time.
**
** The boot allocator redistributes a sub-part of the whole physical memory while
** the physical memory allocator is being set up. When it is done, the boot
** allocator transfers the ownership of the physical memory to the new one, and
** `panic()` is called on any future call.
*/
...
boot_pmm_init(void)
{
	...
	end = (struct multiboot_mmap_entry const *)((uchar const *)mb_mmap + mb_mmap->size);
	...
	while (entry < end) {
		...
		entry = (struct multiboot_mmap_entry const *)((uchar *)entry + mb_mmap->entry_size);
	}
